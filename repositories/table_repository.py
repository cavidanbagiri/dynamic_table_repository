...
class DeleteTableRepository:
    ...
    async def delete_table(self, table_name: str, user_id: int):
        try:
            ...
            await self.db.commit()
            return {"message": "Table deleted successfully", "table_id": table.id}
    ...
    async def create_table(self, user_id: int, file: UploadFile, table_status: str, table_description: str, table_name: str = None, table_category: str = None):
        async with SessionLocal() as session:
            try:
                ...
                await CheckTableNameAvailabilityRepository.check_table_already_exists(define_table_name, session)
                ...
                table_id = await CreateTableDefinitionRepository.create_table_definition(table_status, table_description, define_table_name, table_category, session)
                ...
                await CreateUserRepository.create_user_tables(user_id, table_id, session)
                logging.debug(f"UserTable created for user {user_id} and table {table_id}.")
                ...
                if 'table_id' in locals():
                    await DeleteTableDefinitionRepository.delete_table_definition(table_id)
